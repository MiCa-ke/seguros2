...
<form action="{{route('rol.update', $role->id)}}" method="POST" >
    ...
    <div class="max-w-7xl py-6 px-4 sm:px-6 lg:px-8">
        ...
        <p class="text-lg italic tracking-tight font-sans  ">Editar datos del roles</p>
    </div>

    @method('PUT')
    <input type="text" name="id" value="{{$role->id}}" class="hidden">

    <div class="flex flex-col mt-2">
        <label for="name" class="hidden">Rol</label>
        <input type="nombre" name="name" id="nombre" placeholder="Nombre del rol" value="{{$role->name}}"
                    ...
                    <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                        @foreach ($permissions as $id => $permission)
                            ...
                            <div class="flex items-center">
                                <input id="filter-mobile-color-0" name="permissions[]" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
